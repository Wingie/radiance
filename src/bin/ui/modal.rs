...
fn triangle_to_parallelogram(
    a: [Vector2<f32>; 3],
    b: [Vector2<f32>; 3],
) -> ([Vector2<f32>; 4], [Vector2<f32>; 4]) {
    (
...
pub fn modal_ui(
        ...
        .expect("Tried to display modal but nothing currently shown")
    {
        ModalMemory::ProjectionMapEditor(node_id) => {
            if let Some(NodeProps::ProjectionMappedOutputNode(props)) =
                props.node_props.get_mut(node_id)
            {
                ...
                ComboBox::from_id_salt(0)
                    .selected_text(
                        props
                            .screens
                            .get(projection_map_editor_memory.selected_screen)
                            .map(|s| s.name.as_str())
                            .unwrap_or(&""),
                    )
                    .show_ui(&mut ui, |ui| {
                ...
                ui.with_layout(
                    ...
                    |ui| {
                        ...
                        let left = available
                            .intersect(Rect::everything_left_of(midpoint - 0.5 * SCRIM_PADDING));
                        let right = available
                            .intersect(Rect::everything_right_of(midpoint + 0.5 * SCRIM_PADDING));
                        ...
                        // Left side UI (virtual view)
                        {
                            ...
                            let factor_fit_x = (props.resolution[0] as f32 * left.height()
                                / props.resolution[1] as f32
                                / left.width())
                            .min(1.);
                            let factor_fit_y = (props.resolution[1] as f32 * left.width()
                                / props.resolution[0] as f32
                                / left.height())
                            .min(1.);
                            let left = Rect::from_center_size(
                                left.center(),
                                left.size() * vec2(factor_fit_x, factor_fit_y),
                            );
                            ...
                            for (i, screen) in props.screens.iter_mut().enumerate() {
                                ...
                                let physical_to_virtual =
                                    virtual_to_physical.try_inverse().unwrap(); // Calling unwrap on user data
                                let virtual_uv_to_pt = Matrix3::<f32>::new(
                                    left.max.x - left.min.x,
                                    0.,
                                    left.min.x,
                                    0.,
                                    left.max.y - left.min.y,
                                    left.min.y,
                                    0.,
                                    0.,
                                    1.,
                                ...
                                let physical_uv_to_virtual_uv = |physical_uv: Vector2<f32>| {
                                    let uvw =
                                        Vector3::<f32>::new(physical_uv[0], physical_uv[1], 1.);
                                ...
                                for (j, vertex) in screen.crop.iter().enumerate() {
                                    ...
                                    let handle_rect =
                                        Rect::from_center_size(pt, Vec2::splat(EDITOR_PT_SIZE));

                                    if i == selected_screen_index {
                                        let handle_response = ui.interact(
                                            handle_rect,
                                            Id::new(("virtual handle", j)),
                                    ...
                                    if locked_point_indices.contains(&j) {
                                        ui.painter().rect_filled(
                                            ...
                                            0.,
                                            if i == selected_screen_index {
                                                EDITOR_STROKE_SELECTED.color
                                            } else {
                                                EDITOR_STROKE_BLURRED.color
                                            },
                                        );
                                    }
                                ...
                                if let Some((handle_index, delta_uv)) = handle_drag {
                                    ...
                                    let locked_physical_uvs: Vec<_> = locked_point_indices
                                        .iter()
                                        .map(|&j| screen.crop[j])
                                        .collect();

                                    // Convert physical to virtual UVs and move the dragged one:
                                    let mut virtual_uvs: Vec<_> = screen
                                        .crop
                                        .iter()
                                        .cloned()
                                        .map(physical_uv_to_virtual_uv)
                                        .collect();
                                    virtual_uvs[handle_index] += delta_uv;

                                    // Now get the locked virtual UVs post-drag:
                                    let locked_virtual_uvs: Vec<_> = locked_point_indices
                                        .iter()
                                        .map(|&j| virtual_uvs[j])
                                        .collect();

                                    // Compute the new perspective matrix that keeps the locked points still
                                    let (locked_virtual_uvs, locked_physical_uvs) =
                                        match locked_point_indices.len() {
                                            3 => triangle_to_parallelogram(
                                                locked_virtual_uvs.try_into().unwrap(),
                                                locked_physical_uvs.try_into().unwrap(),
                                            ),
                                            4 => (
                                                locked_virtual_uvs.try_into().unwrap(),
                                                locked_physical_uvs.try_into().unwrap(),
                                    ...
                                    let virtual_to_physical =
                                        four_point_mapping(locked_virtual_uvs, locked_physical_uvs)
                                            .unwrap();
                                    screen.map = virtual_to_physical;

                                    // Re-compute the physical UVs from the virtual ones so all the virtual ones stay still
                                    screen.crop = virtual_uvs
                                        .into_iter()
                                        .map(|virtual_uv: Vector2<f32>| {
                                            let uvw = Vector3::<f32>::new(
                                                virtual_uv[0],
                                                virtual_uv[1],
                                                1.,
                                            );
                        ...
                        // Right side UI (physical view)
                        {
                            ...
                            let factor_fit_x = (screen.resolution[0] as f32 * right.height()
                                / screen.resolution[1] as f32
                                / right.width())
                            .min(1.);
                            let factor_fit_y = (screen.resolution[1] as f32 * right.width()
                                / screen.resolution[0] as f32
                                / right.height())
                            .min(1.);
                            let right = Rect::from_center_size(
                                right.center(),
                                right.size() * vec2(factor_fit_x, factor_fit_y),
                            );
                            ...
                            for (i, vertex) in screen.crop.iter().enumerate() {
                                ...
                                let last_pt = physical_uv_to_pt(last_uv);
                                ui.painter()
                                    .line_segment([last_pt, pt], EDITOR_STROKE_SELECTED);
                            }
                            ...
                            if let Some((handle_index, delta_uv)) = handle_drag {
                                ...
                                let locked_virtual_uvs: Vec<_> = locked_point_indices
                                    .iter()
                                    .map(|&j| physical_uv_to_virtual_uv(screen.crop[j]))
                                    .collect();
...
pub fn modal<'a>(
    ...
    move |ui: &mut Ui| modal_ui(ui, id, props, node_states, preview_images)
}
